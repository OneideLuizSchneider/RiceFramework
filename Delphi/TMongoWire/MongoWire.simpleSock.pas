...
begin
  ...
  if MongoWire.simpleSock.connect(FSocket,FAddr,SizeOf(TSocketAddress))=SOCKET_ERROR then
    RaiseLastWSAError
  else
    FConnected:=true;
end;
...
begin
  ...
  if MongoWire.simpleSock.bind(FSocket,a,SizeOf(TSocketAddress))=SOCKET_ERROR then
    RaiseLastWSAError;
end;
...
begin
  //call bind first!
  if MongoWire.simpleSock.listen(FSocket,SOMAXCONN)=SOCKET_ERROR then
    RaiseLastWSAError;
end;
...
begin
  ...
  FillChar(a,l,#0);
  Result:=TTcpSocket.Create(FFamily,MongoWire.simpleSock.accept(FSocket,@a,@l));
  Result.FAddr:=a;
end;
